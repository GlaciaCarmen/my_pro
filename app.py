...
def water_quality_data():
    ...
    if site not in monitoring_sites or param not in water_parameters:
        return jsonify({'error': 'Invalid parameters'}), 400

    site_data = all_water_data[all_water_data['数据来源_监测点'] == site]
    time_series = site_data['监测时间'].dt.strftime('%Y-%m-%d').tolist()
    values = site_data[param].tolist()
